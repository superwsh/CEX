...
public class WithdrawController {
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult withdraw(@SessionAttribute(SESSION_MEMBER) AuthMember user, String unit, String address,
                                  BigDecimal amount, BigDecimal fee, String remark) throws Exception {

        log.info(String.format("start withdraw call %s,%s,%s,%s,%s,%s", user, unit, address, amount, fee, remark));
        Coin coin = coinService.findByUnit(unit);
        amount.setScale(coin.getWithdrawScale(), BigDecimal.ROUND_DOWN);
        notNull(coin, sourceService.getMessage("COIN_ILLEGAL"));
        //教学项目，省去密码校验
        isTrue(coin.getStatus().equals(CommonStatus.NORMAL) && coin.getCanWithdraw().equals(BooleanEnum.IS_TRUE), "COIN_NOT_SUPPORT");
        isTrue(compare(fee, new BigDecimal(String.valueOf(coin.getMinTxFee()))), "CHARGE_MIN:" + coin.getMinTxFee());
        isTrue(compare(new BigDecimal(String.valueOf(coin.getMaxTxFee())), fee), "CHARGE_MAX:" + coin.getMaxTxFee());
        isTrue(compare(coin.getMaxWithdrawAmount(), amount), "WITHDRAW_MAX:" + coin.getMaxWithdrawAmount());
        isTrue(compare(amount, coin.getMinWithdrawAmount()), "WITHDRAW_MIN" + coin.getMinWithdrawAmount());
        MemberWallet memberWallet = memberWalletService.findByCoinAndMemberId(coin, user.getId());
        isTrue(compare(memberWallet.getBalance(), amount), "INSUFFICIENT_BALANCE");
