...
//        && StringUtils.isNotEmpty(current.getTxid())
        if (current != null ) {
            synchronized (current) {
                try {
                    checkTimes ++;
                    if (isTransactionSuccess(current.getTxid())) {
    ...
    private boolean isTransactionSuccess(String txid) throws Exception {
        var receipt = web3j.ethGetTransactionReceipt(txid).send();
        return receipt.getTransactionReceipt().map(r -> "0x1".equals(r.getStatus())).orElse(false);
    }
}
