...
public class FinanceConsumer {
    ...
    public void handleWithdraw(ConsumerRecord<String, String> record) {
        ...
        try {
            String serviceName = "service-rpc-" + record.key();
//            String url = "http://" + serviceName.toUpperCase() + "/" + record.key().toLowerCase() + "/rpc/withdraw?address={1}&amount={2}&fee={3}";
            String url = "http://" + serviceName.toLowerCase() + "/" + record.key().toLowerCase() + "/rpc/withdraw?address={1}&amount={2}&fee={3}";

            Coin coin = coinService.findByUnit(record.key());
            logger.info("coin = {},url={}", coin.toString(),url);
            //去掉自动提币的判断 默认都能提币
            if (coin != null) {
